...
class job extends Front_Controller {
    ...
    public function jobDetails($job_id) {
        ...
        // Check whether the Job is accessible. 
        if($this->job_model->isAccessible($jobinfo['id'],$jobinfo['company_id'],$jobinfo['job_status_id'])==FALSE)
        {
            $this->session->set_flashdata('error','Invalid URL. Please check URL or contact to our help desk.');
    ...
    public function apply() {
        ...
        if (!empty($job_id) && !empty($uid)) {
            ...
            if (isset($_POST['email'])) {
                ...
                // Send an email notification JINGJOBS Admin about the Application.
                
                $this->email->from('do-not-reply@jingjobs.com', 'JingJobs');

                $this->email->to('samantha@jingjobs.com','kwoksamantha@hotmail.com','justinwaulters@yahoo.com');

                $this->email->subject('Job Application: '.$job_arr['job_name']);
                
                $this->email->message('
                    <p><b>Job Seeker Name:</b>'.$this->session->userdata('first_name').' '.$this->session->userdata('last_name').'</p>
                    <p><b>Job Title:</b>'.$job_arr['job_name'].'</p>
                    <p><b>Job Link:</b>http://jingjobs.com/job/jobDetails/'.$job_arr['id'].'</p>');
                $this->email->send();
            }
    ...
    public function bookmarkCompany() {
        ...
        if (!empty($_POST['company_id']) && !empty($uid)) {
            // do bookmark
            $this->load->model('job_model');

            $result['status'] = $this->job_model->bookmarkCompany($_POST['company_id'], $uid);
            $result['status'] = $result['status'] ? 'success' : 'failed.';
