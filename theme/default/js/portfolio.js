...
function openPortfolioDetails(){
    ...
    $('#portfolio_view_bar ul li img').each(function(){
        var self = $(this);
        ...
        new_img.onload = function() {
            ...
            var orgRatio = 99 / 105;
            var tempRatio = img_width / img_heigth;
            if(tempRatio > orgRatio)
            {
                var tempWidth = 105 * (img_width / img_heigth);
                var left = (99 - tempWidth) / 2;
                self.css({width: 'auto', height: 105+'px', left: left+'px'});
            }
            else
            {
                var tempHeight = 99 * (img_heigth / img_width);
                var top = (105 - tempHeight) / 2;
                self.css({width: 99+'px', height: 'auto', top: top+'px'});
            }
        }
    ...
    if(len > 0){
        ...
        for(var i = 0; i < len; i++){
            ...
            var itemParameterStr = 'project-name="' + portfolio_projects[i]['name']
                ...
                + '" file-url="' + site_url + 'attached/workExamples/500/' + portfolio_projects[i]['file_url'] + '">';



            htmltext = htmltext + '<a style="position:relative;" href="javascript:void(0);" class="portfolio_item" ' + itemParameterStr;
            ...
            switch(portfolio_projects[i]['type']){
                ...
                case '1':
                    ...
                    imgHtml = "<img src='" + site_url + "attached/workExamples/200/" + portfolio_projects[i]['file_url']  +"'/>";
                     $(imgHtml).load(function() {
                        var orgRatio = 134 / 80;
                        var tempRatio = $(this).width() / $(this).height();
                        if(tempRatio > orgRatio)
                        {
                            var tempWidth = 80 * ($(this).width() / $(this).height());
                            var left = (134 - tempWidth) / 2;
                            $(this).css({width: 'auto', height: 80+'px', left: left+'px'});
                        }
                        else
                        {
                            var tempHeight = 134 * ($(this).height() / $(this).width());
                            var top = (84 - tempHeight) / 2;
                            $(this).css({width: 134+'px', height: 'auto', top: top+'px'});
                        }
                    });

                    navigatorBarImgHtml = "<img src='" + site_url + "attached/workExamples/200/" + portfolio_projects[i]['file_url']  +"'/>";
        ...
        */
        ...
        htmltext = '<span class="als-prev"><img src="' + theme_url + 'style/btns/previous_arrow.png" alt="prev" title="previous"/></span><div class="als-viewport"><div class="profile_portfolios als-wrapper"">'
                    ...
                    + '</div><span id="tempFixNext0" class="als-next"><img src="' + theme_url + 'style/btns/forward_arrow.png" alt="next" title="next"  /></span>';
        }
        ...
        $('#portfolio_list').append('<span class="als-next"><img src="' + theme_url + 'style/btns/forward_arrow.png" alt="next" title="next" /></span>');
        ...
        navigatorBarHtml = '<span class="als-prev"><img src="' + theme_url + 'style/btns/previous_arrow.png" alt="prev" title="previous" /></span><div class="als-viewport"><ul class="als-wrapper">'
                            ...
                            + '</ul></div><span id="tempFixNext" class="als-next"><img src="' + theme_url + 'style/btns/forward_arrow.png" alt="next" title="next" /></span>';
        ...
        $('#portfolio_view_bar').append('<span class="als-next"><img src="' + theme_url + 'style/btns/forward_arrow.png" alt="next" title="next" /></span>');
...
$(function(){
    ...
    $('.add_portfolio_project_btn').click(function(){
        ...
        if(type == -1)
        {
            // alert("Please upload a file firstly before you save.");
            // getUploadedFileType already returned an error message "Wrong file name" from common.js
            return false;
        }
        ...
        $.post(site_url + 'jobseeker/addPortfolioProject',
            ...
            function(result, status){
                ...
                $('#project_desc').val('');
                $('#portfolio_file_name').val('');// Reset last uploaded file name from Hidden Field.
                //we should re-create the portfolio list here to keep the list structure
    ...
    $('.edit_portfolio_pop_close').click(function(){
        ...
        $('#upload_portfolio_file_status_message').html('');
    });
})